    ...
    for enrollment in CourseEnrollment.enrollments_for_user(user):
        ...
        if (not course_overview) or isinstance(course_overview, ErrorDescriptor):
            log.error(
                ...
                user.username,
                "broken" if course_overview else "non-existent",
        ...
        elif course_org_filter and course_overview.location.org != course_org_filter:
            continue
            ...
            if course_overview.has_any_active_web_certificate:
                status_dict.update({
                    ...
                    'cert_web_view_url': u'{url}'.format(
                        url=get_certificate_url(user_id=user.id, course_id=unicode(course_overview.id))
                    )
                })
            else:
